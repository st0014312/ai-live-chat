...
def create_knowledge_base_agent(
    ...
    try:
        ...
        temperature = temperature or float(os.getenv("KB_AGENT_TEMPERATURE", DEFAULT_TEMPERATURE))
        ...
        logger.info(f"Creating knowledge base agent with model: {model_name}, provider: {embeddings_provider}")
        ...
        raise KnowledgeBaseAgentError(f"Knowledge base agent creation failed: {str(e)}") from e
...
def _initialize_embeddings(provider: str = "huggingface") -> Any:
    ...
    try:
        if provider == "openai":
            ...
            if not api_key:
                raise KnowledgeBaseAgentError("OPENAI_API_KEY environment variable not set")
        ...
        raise KnowledgeBaseAgentError(f"Embedding initialization failed: {str(e)}") from e
...
def _initialize_knowledge_base(
    ...
    try:
        ...
        kb_path = Path(knowledge_base_dir or os.getenv("KNOWLEDGE_BASE_DIR", "./knowledge_base"))
        
        if not kb_path.exists():
            raise KnowledgeBaseAgentError(f"Knowledge base directory not found: {kb_path}")
        ...
        raise KnowledgeBaseAgentError(f"Knowledge base initialization failed: {str(e)}") from e
