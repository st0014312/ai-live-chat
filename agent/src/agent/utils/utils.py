...
def initialize_llm(
    ...
    try:
        if llm_provider == "openrouter":
            ...
            if not base_url:
                raise LLMInitializationError(
                    "OPENROUTER_BASE_URL environment variable not set"
                )
        ...
        logger.debug(
            f"LLM initialized: {model_name} (provider: {llm_provider}, temperature: {temperature})"
        )
        return llm
...
def initialize_sql_database(
    ...
    try:
        ...
        raise DatabaseInitializationError(
            f"Failed to download database schema: {str(e)}"
        ) from e
...
def pretty_print_messages(
    update: Union[Dict[str, Any], tuple], last_message: bool = False
) -> None:
    """
