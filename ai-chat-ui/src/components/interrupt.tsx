...
export function Interrupt({ event, resolve }: InterruptProps) {
  ...
  const wrapperStyles =
    "flex flex-col justify-center items-center h-full w-full bg-indigo-600/50 my-4 rounded-xl";
  ...
  const textareaStyles =
    "w-full p-2 rounded-xl focus:outline-purple-500 border shadow-inner min-h-[100px] bg-white";
  const buttonStyles =
    "px-4 py-2 bg-black text-white rounded-xl hover:bg-gray-700 transition-colors";
    ...
    <div className={wrapperStyles}>
      <div className={contentStyles}>
        ...
        <div className="flex justify-center mt-4">
          <button onClick={() => resolve(event.value)} className={buttonStyles}>
            Respond
          </button>
        </div>
      </div>
